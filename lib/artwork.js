...
const { getGeolocationForArtwork } = require('./geocoding-service.js');
...
class Artwork {
  ...
  writeArtworkIntoFile(filePath) {

  }

  generateCoordinates() {
    getGeolocationForArtwork(this)
      .then(geolocation => {
        // artwork.geometry = artwork.geometry || [];
        this.geometry.coordinates = [geolocation.long, geolocation.lat];
        if (!this.geometry.coordinates) {
          this.errors = artwork.errors || [];
          this.errors.push('coordinates');
        }
        return artwork;
      })
      .catch(e => {
        console.error(
          `\nError finding geolocation for '${this.properties.title}'; query '${
  ...
  cleanDescription() {
    ...
    // /.*((?i)artist(?-i))s?:.*\(.*\d{4}\)\\r\\n/g
    [/[^]*Artists?:.*\(.*\d{4}\)/i, /District:[^]+/im].forEach(rule => {
      if (description) {
