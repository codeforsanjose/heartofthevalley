...
class ArtworkScraper {
  ...
  static mergeArtworksIntoFile(destinationPath, artworkFilePaths) {
    ...
    artworkFilePaths.forEach(filePath => {
      ...
      fileArtworks.forEach(artwork => {
        let { title, artist } = artwork.properties;
        let key = title;
  ...
  constructor({ numArtworksToScrape }) {
...
// ArtworkScraper.mergeArtworksIntoFile(CONSTANTS.PATH_CONSOLIDATED_ARTWORKS, [
