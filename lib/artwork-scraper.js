...
let legacyArt = require(PATHS.PATH_LEGACY_ARTWORKS);
const Artwork = require('./artwork');
const stringHash = require('string-hash');

class ArtworkScraper {
  static generateID(artwork) {
    const { artist, title, description, address } = artwork.properties || artwork;
    
    return stringHash(`${artist}-${title}-${address}-${description}`);
  }
  ...
  static mergeArtworksIntoFile(destinationPath, artworkFilePaths) {
    ...
    artworkFilePaths.forEach(filePath => {
      ...
      for (let artwork of fileArtworks) {
        ...
        if (map[id]) {
          console.log(`'${map[id].properties.title}' was overriden`);
          numOverrides++;
        }
        map[id] = artwork;
      }
