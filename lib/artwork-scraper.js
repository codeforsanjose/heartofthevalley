...
class ArtworkScraper {
  ...
  scrapeArtworks() {
    const setMap = {
      ...
      rawAddress: 'span.item_value a@title',
      rawDescription: '.detail-content',
      image: 'span.facility_item_photo img@src'
    };
    ...
    return new Promise(resolve => {
      require('osmosis')
        ...
        .paginate('div.list-pager > a.pg-selected + a.pg-normal:not(.pg-button)[href]', 5) // follow pagination 5 times
        .find('h2 > a.facility_item_name') // selector for links to individual pages about art work
        .set({ sourceURL: '@href' })
        .then((_content, data) => {
        ...
        .data(artwork => {
          ...
          artwork = Object.assign(artwork, {
            sourceOfInformation: `${CONSTANTS.URL_SEARCH}`,
            image: artwork.image ? CONSTANTS.URL_SAN_JOSE_GOV + artwork.image : null,
            streetAddress: artwork.rawAddress || null,
          });
          artwork = new Artwork(artwork);
          // artwork.getArtistFromDescription(); no need to get artist from description
          artwork.cleanArtist();
          artwork.cleanTitle();
          artwork.cleanDescription();
          artwork.parseStreetAddress();
          artwork.generateID();
