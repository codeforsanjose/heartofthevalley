...
class ArtworkScraper {
  constructor(numArtworksToScrape = 1) {
    this.numArtworksToScrape = numArtworksToScrape;
    this.numScraped = 0;
  ...
  scrapeArtworks() {
    ...
    return new Promise(resolve => {
      ...
      const promises = artworks.reduce((arr, artwork) => {
        ...
        if (this.legacyArtMap[artwork.properties.title]) {
  ...
  writeData(data) {
    return new Promise(resolve => {
      fs.writeFile(CONSTANTS.PATH_ARTWORKS_SCRAPED, JSON.stringify(data), { flags: 'r+' }, err => {
        if (err) {
          throw err;
        }
        console.log(`\nArtworks scraped: ${this.numScraped}`);
        console.log(`New artworks that do not exist in 'art.js' file: ${this.numNewArtworks}`);
        console.log(`Legacy artworks merged: ${this.numLegacyArtworks}`);
        console.log(`Total artworks: ${this.numNewArtworks + this.numLegacyArtworks}`);
        console.log(`Scraped artworks with errors: ${this.numErrors}`);
        console.log(`All artworks written to: '${CONSTANTS.PATH_ARTWORKS_SCRAPED}'`);
        resolve(data);
      });
    }).then(data => {
      ...
      fs.writeFile(
        ...
        err => {
          ...
          console.log(
            `${artworkErrors.length} scraped errors written to: '${CONSTANTS.PATH_ARTWORKS_ERRORS}'`
          );
        }
      );
    });
